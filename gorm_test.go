...
func TestUpdate(t *testing.T)  {
	...
	user.Name.FirstName = "Fahril"
	user.Name.MiddleName = ""
	user.Name.LastName = "Hadi"
...
func TestPreloadManyToMany(t *testing.T)  {
	var product Product
	err := db.Preload("LikeUsers").Take(&product, "id = ?", "P001").Error
	assert.Nil(t, err)
	assert.Equal(t, 2, len(product.LikeUsers))
}
...
func TestAssociationFind(t *testing.T)  {
	...
	var users []User
	err = db.Model(&product).Where("first_name LIKE ?", "User%").Association("LikeUsers").Find(&users)
	assert.Nil(t, err)
	assert.Equal(t, 1, len(users))
}
...
func TestAssociationAppend(t *testing.T)  {
	...
	err = db.Model(&product).Association("LikeUsers").Append(&user)
	assert.Nil(t, err)
}
...
func TestAssociationDelete(t *testing.T)  {
	...
	err = db.Model(&product).Association("LikeUsers").Delete(&user)
	assert.Nil(t, err)
}
...
func TestAssociationClear(t *testing.T)  {
	...
	err = db.Model(&product).Association("LikeUsers").Clear()
	assert.Nil(t, err)
}
